...
/**
 * Load template files of the plugin also include a filter v_get_template_part<br>
 ...
 * @return string
 */
function v_get_template_part($slug, $name = '', $include = true)
{
    $template = '';
    ...
    // Look in yourtheme/slug-name.php and yourtheme/viper/slug-name.php
    if ($name) {
    ...
    // If template file doesn't exist, look in yourtheme/slug.php and yourtheme/viper/slug.php
    if (!$template) {
        $template = locate_template(array("{$slug}.php", $plugin_slug."{$slug}.php"));
    }

    // Allow 3rd party plugin filter template file from their plugin
    $template = apply_filters('v_get_template_part', $template, $slug, $name);
...
/**
 * Load email template files of the plugin also include a filter v_get_email_template<br>.
 * 
 * @param string $name
 * @param string $prefix
 *
 * @return string
 */
function v_get_email_template($name, $prefix = '')
{
    $template = '';
    ...
    // Look in yourtheme/viper/{locale}/name.tpl and yourtheme/viper/email-templates/{locale}/name.tpl
    if (empty($template)) {
        $search = array(
    ...
    // Allow 3rd party plugin filter template file from their plugin
    $template = apply_filters('v_get_email_template', $template, $name, $prefix);
